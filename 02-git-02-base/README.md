...
Если все проделанно правильно, то результат команды `git remote -v` должен быть следующий:
```bash
$ git remote -v
...
Выполните push локальной ветки `master` в новые репозитории. 
Подсказка: `git push -u gitlab master`. На этом этапе история коммитов во всех трех репозиториях должна совпадать. 

## Задание №2 – Теги.

Представьте ситуацию, когда в коде была обнаружена ошибка, надо вернуться на предыдущую версию кода,
исправить ее и выложить исправленный код в продакшн. Никуда код выклаыдвать не будем, но пометим некоторые коммиты тегами и 
...
Давайте посмотрим как будет выглядеть историю коммитов при создании веток. 
...
1. Теперь измените содеражние файла `README.md` добавив новую строчку.
1. Отправьте изменения в репозиторий, посмотрите как изменится схема на странице https://github.com/YOUR_ACCOUNT/devops-netology/network 
и как измениться вывод команды `git log`.

## Задание №4 – Упрощаем себе жизнь.

Давайте попробуем поработь с Git при помощи визуального редактора. 

1. В используемой нами IDE Py Charm откройте визуальный редактор работы с git находящий в меню View -> Tool Windows -> Git.
