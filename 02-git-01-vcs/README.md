...
1. Выполните команду `git push`, если git запросит логин и пароль – введите ваш логин и созданный personal access token от github. Перейдите по https://github.com/settings/tokens/new, напишите название токена и после выберите те права доступа, которыми будет обладать данный токен. Так же можно указать срок годности токена. Если это не требуется, то выберите "no expiration". Сохраните себе полученный токен - отныне это ваш пароль для доступа к github api.
