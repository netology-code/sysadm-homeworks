# Домашнее задание к занятию «2.3. Ветвления и слияния Git»
...
2. Создадим коммит, например с описанием `prepare for merge and rebase` и отправим его в ветку мастер. 
...
```
1. Создайте коммит `merge: @ instead *` отправьте изменений в репозиторий.
1. И разработчик подумал и решил внести еще одно изменением в `merge.sh` 
```bash
#!/bin/bash
# display command line options
...
2. Создайте коммит `merge: use shift` отправьте изменений в репозиторий. 

#### Изменем master.
1. Вернитесь в ветку `master`. 
2. И предположим, что кто-то пока мы работали над веткой `git-merge` изменил `master`, для этого
...
#### Подготовка файле rebase.sh.
1. Предположим, что теперь другой участник нашей команды 
не сделал `git pull`, либо просто хотел ответвится не от 
...
1. И сделаем еще один коммит `git-rebase 2` с пушем заменив `echo "Parameter: $param"` 
...
а с rebase бывают сложности, поэтому давайте смержим все наработки в `master` и разрешим конфилкты. 

Если все было сделано правильно, то на странице `network` в титхабе находящейся по адресу 
`https://github.com/ВАШ_ЛОГИН/ВАШ_РЕПОЗИТОРИЙ/network` будет примерно такая схема:
![Созданы обе ветки](img/01.png)

#### Merge
Сливаем ветку `git-merge` в мастер и отправяем изменения в репозиторий, должно получиться без конфликтов:
```bash
$ git merge git-merge
...
#### Rebase
1. А перед мержем ветки `git-reabse` выполним ее `rebase` на мастер. Да, мы специально создали
ситуацию с конфликами, что бы потренероваться их решать. 
1. Переключаемся на ветку `git-reabse` и выполняем `git rebase -i origin/master`. 
В открывшемся диалоге должно быть два выполненных нами коммита, давайте заодно объединим их в один, 
указав слева от нижнего `fix`. 
...
``` 
Если посмотреть содержимое файла `rebase.sh` то увидем метки оставленные гитом для решения конфликта:
```bash
cat rebase.sh
#!/bin/bash
# display command line options
...
```
И попробуем выполнить `git push`, либо `git push -u origin git-rebase` что бы точно указать что и куда мы хотим запушить. 
...
Что бы гит позволил нам это сделать давайте добавить флаг `force`:
```bash
...
Цель задания была попробовать на практике как выглядит решение конфликтов.
На самом деле, обычно при нормальном ходе разработки выполнять `rebase` достаточно просто, 
что позволяет объединить множество промежуточных коммитов при решении задачи, что бы 
не засорять историю, поэтому многие команды и разработчики предпочитают такой способ.   


Есть еще такой тренажер https://learngitbranching.js.org/, где можно потренероваться работе с 
