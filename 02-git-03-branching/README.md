...
#### Merge
1. Сливаем ветку `git-merge` в мастер и отправяем изменения в репозиторий, должно получиться без конфликтов:
```bash
$ git merge git-merge
...
#### Rebase
1. А ветку `git-reabse` давайте не смержим, а выполним ее `rebase` на мастер. Да, мы специально создали
