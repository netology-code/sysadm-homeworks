...
Для того, чтобы лучше понимать предназначение дальнейших инструментов, с которыми нам предстоит работать, давайте 
...
От клиентов приходят задачи, связанные с разработкой нового функционала - например, для вашего интернет-магазина. Задач много, 
они требуют выкладки на тестовые среды, одобрения тестировщика, проверки менеджером перед показом клиенту. В случае необходимости будет необходим откат изменений. 

Опишите, как вы видите процесс решения задачи. <br> 
Использование какого-либо конкретного метода разработки не обязательно - главное прописать по пунктам шаги трансформации задачи в конечный результат с участием менеджера, разработчика (или команды разработчиков), тестировщика (или команды тестировщиков) и себя как DevOps-инженера.
